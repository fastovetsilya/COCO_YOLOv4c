...
def save_yolo_data(merged_annotations, obj_categories, image_id, images_path, output_path):
    ...
    image_filename = merged_annotations[merged_annotations['image_id'] == image_id]['file_name'].iloc[0].replace('.jpg', '.txt')    
    img_obj_categ = np.array(merged_annotations[merged_annotations['image_id'] == image_id]['category_id'])
    img_obj_categ = np.array([obj_categories[1, np.where(obj_categories == img_obj_categ[i])[1][0]] for i in range(len(img_obj_categ))])
    
    # For original rbbx
    # image_bbx = list(merged_annotations[merged_annotations['image_id'] == image_id]['bbox'])
    
    # For cbbx
    image_polygon = np.array(merged_annotations[merged_annotations['image_id'] == image_id]['segmentation'])
    image_bbx = [cbbx_from_poly(el) for el in image_polygon if type(el) is list]
    ...
    for yolo_object in range(len(image_bbx_yolo)):
        ...
        image_bbx_yolo_object = [img_obj_categ[yolo_object]] + image_bbx_yolo[yolo_object]
        image_bbx_yolo_object = str(image_bbx_yolo_object)
        image_bbx_yolo_object = image_bbx_yolo_object.replace(',', '').replace('[', '').replace(']', '')
    ...
    copyfile(images_path + image_filename.replace('.txt', '.jpg'), output_path + image_filename.replace('.txt', '.jpg'))
    
    
    
def cbbx_from_poly(obj_polygons):
    merged_contours = np.array(obj_polygons[0]).reshape((-1,1,2)).astype(np.int32)
    
    if len(obj_polygons) > 1:
        for i in range(1, len(obj_polygons)):
            obj_polygon_contours = np.array(obj_polygons[i]).reshape((-1,1,2)).astype(np.int32)
            merged_contours = np.concatenate((merged_contours, obj_polygon_contours), 0)
