...
annotations_train_merged = pd.merge(pd.DataFrame(annotations_train['images']), 
                       pd.DataFrame(annotations_train['annotations']), 
...
55 - orange
'''
obj_categories = [37, 53, 55]
obj_categories = np.array([obj_categories, 
                          [i for i in range(len(obj_categories))]])

annotations_train_merged = annotations_train_merged[annotations_train_merged['category_id'].isin(obj_categories[0, :])]
annotations_val_merged = annotations_val_merged[annotations_val_merged['category_id'].isin(obj_categories[0, :])]
...
for s in range(len(train_images_list)):
    train_images_list[s] = train_images_list[s].replace('.txt', '.jpg').replace('./yolo_data/train/', 'data/obj_' + yolo_obj_folder_name + '/')
...
for s in range(len(val_images_list)):
    val_images_list[s] = val_images_list[s].replace('.txt', '.jpg').replace('./yolo_data/val/', 'data/obj_' + yolo_obj_folder_name + '/')
